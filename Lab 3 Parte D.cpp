...
// ==== Sensor Capacitivo ====
constexpr uint8_t TOUCH_PIN = 13;          // Pin táctil (Touch4)
constexpr uint16_t TOUCH_THRESHOLD = 30;   // Umbral para detectar toque
...
// ==== Reinicio del contador automático usando sensor ====
  if (touchRead(TOUCH_PIN) < TOUCH_THRESHOLD) {
    cuentaTimer = 0;
    mostrarBinarioTimer(cuentaTimer);
    delay(200); // Antirrebote para el touch
  }
}